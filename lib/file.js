...
function File(options) {
  ...
  if (!self.meta) {
    if (self.service) {
      self.meta = self.service;
      
      if (self.provider) {
        self.meta += ':' + self.provider;
        
        if (self.owner) {
          self.meta += ':' + self.owner;
        
          if (self.repository) {
            self.meta += ':' + self.repository;
          }
        }
      }
    }
  ...
  if (self.fullpath && !noIndex.util.isArray(self.path)) {
...
File.parse = function(src) {
    ...
    if (p) {
      if (p[5]) {
        ...
        options = {
          ...
          fullpath: fullpath ? (fullpath[1] || fullpath[2]) : null,
        };

        options.meta = options.service + ':' + options.provider + ':' + options.owner + ':' + options.repository;
      } else {
        options = {
          service: p[6] ? p[6] : '?',
          provider: p[7] ? p[7] : '?',
      ...
      if (p) {
        ...
        options.meta = options.service + ':' + options.provider + ':' + options.owner;
      } else {
        p = (/^(.*?):(.*)|^(.*)/i).exec(src);
        ...
        if (p) {
          ...
          if (options.provider) {
            options.meta = options.service + ':' + options.provider;
          } else {
            options.meta = options.service;
          }
        }
      }
    }
  ...
  if (options.fullpath) {
    options.path = options.fullpath.split('/');
  }
